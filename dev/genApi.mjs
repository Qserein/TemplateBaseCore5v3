...
const groups = {
  main(o) {
    ...
    o.defines = ['AdminAuth', 'AdminSystem', 'AdminMain', 'Common', 'NoGroup']
  },
}
...
async function genGroup(gname, gf) {
  ...
  function prefix() {
    return `[${moment().format('HH:mm:ss.SSS')}]`.cyan + `[${gname}]`.green
  }

  /** @param {string} def * @return {Promise<[any, string] | undefined>} */
  async function downloadDefine(def) {
    console.log(prefix(), 'get'.yellow, def)
    const url = `${g.baseUrl}/swagger/${def}/swagger.json`
    try {
      const r = await (await fetch(url)).json()
      console.log(prefix(), 'got'.blue, url)
      return [r, def]
    } catch (e) {
      console.log(prefix(), 'get'.red, url, `failed`.red)
      console.error(e)
    }
  }
  ...
  const ahttpImports = aImports.size > 0 ? `import { AHttpConfig, ${[...aImports].map(i => `a${i}`).join(', ') || ''} } from '${ahttpPath}'` : ''
  ...
  const apis = seq(impls)
    .flatMap(([path, impls]) => {
      const codes = impls.flatMap(([method, { summary, params, body, ret, pathParams }]) => {
        ...
        let args =
          [body ? `body: ${body}` : '', params ? `${pathParams.length > 0 ? `{ ${pathParams.join(', ')}, ...params }` : 'params'}: ${caml_method}Params[${JSON.stringify(path)}]` : '', 'config?: (RequestInit & AHttpConfig)'].filter(i => i).join(', ') || ''
        ...
        const req = `a${method}(${JSON.stringify(path)}${pathReplace}, ${reqArg}, config)`
  ...
  const apis2 =
    Object.entries(items)
      .flatMap(([method, { params, impl }]) => {
        ...
        const [method_head, ...method_body] = method
        const caml_method = `${method_head.toUpperCase()}${method_body.join('')}`
        // const defs = def.flatMap(({ summary, path, params, body, ret }) => {
        //   let args = [body ? `body: ${body}` : '', params ? `params: ${caml_method}Params[${JSON.stringify(path)}]` : '', 'config?: (RequestInit & AHttpConfig)'].filter(i => i).join(', ') || ''
        //   return [summary, ...`export function ${method}(path: ${JSON.stringify(path)}${args ? ', ' : ''}${args}): Promise<${ret}>`.split('\n')].filter(i => i)
        // })
        const impls = impl.flatMap(({ summary, path, params, body, ret, pathParams }) => {
          if (summary) summary = `  ${summary}`
          let args =
            [body ? `body: ${body}` : '', params ? `${pathParams.length > 0 ? `{ ${pathParams.join(', ')}, ...params }` : 'params'}: ${caml_method}Params[${JSON.stringify(path)}]` : '', 'config?: (RequestInit & AHttpConfig)'].filter(i => i).join(', ') || ''
          // let args = ['path: string', body ? `body: any` : '', params ? (pathParams.length > 0 ? `${(() => `{ ${pathParams.join(', ')}, ...params }`)()}: any` : `params: any`) : '', 'config: any'].filter(i => i).join(', ') || ''
          //''.replace()
          const pathReplace = params && pathParams.length > 0 ? pathParams.map(i => `.replace(${JSON.stringify(`{${i}}`)}, \`\${${i}}\`)`).join('') : ''
          const reqArg = method == 'get' ? `${params ? 'params as any' : 'void 0'}` : `${body ? 'body as any' : 'void 0'}, ${params ? 'params as any' : 'void 0'}`
          const req = `a${method}(${JSON.stringify(path)}${pathReplace}, ${reqArg}, config)`
          return [summary, `  [${JSON.stringify(path)}]: (${args}) : Promise<${ret}> => ${req},`]
        })
        const params_code = params.filter(i => i.params).length > 0 ? [`export type ${caml_method}Params = {`, ...def_params, '}'] : []
        return [...params_code, `export const ${method} = {`, ...impls, `}`]
      })
      .join('\n') || ''
  ...
  console.log(prefix(), 'write'.green)
}
