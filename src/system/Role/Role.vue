...
import { Role } from '../../api/gen2/main/types/AdminSystem'
import { api } from '../../api'
...
async function doQuery() {
  const res = await api.main['/api/Admin/Role/List'].get({
...
function selectionChange(rows: Role[]) {
  selected.value = rows
}
...
const editItem = ref<Role>(defaultRole())
const showEdit = ref(false)
const isEdit = ref(true)
function openNew() {
  editItem.value = defaultRole()
  isEdit.value = false
  showEdit.value = true
}
async function openEdit(row: Role) {
  const res = await api.main['/api/Admin/Role/{ID}'].get({ ID: row.ID })
  if (res.success) {
...
async function saveEdit() {
  ...
  if (isEdit.value) {
    const res = await api.main['/api/Admin/Role'].put(data)
    if (res.success) {
      closeEdit()
    ...
    const res = await api.main['/api/Admin/Role'].post(data)
    if (res.success) {
      closeEdit()
...
<template>
  ...
  <el-table class="app-table" ref="tableRef" :data="list" row-key="ID" highlight-current-row tooltip-effect="dark" header-cell-class-name="app-table-header" border height="100%" @selection-change="selectionChange">
    ...
    <el-table-column label="顺序" align="center" prop="Sorting" width="120"></el-table-column>
  ...
  <el-dialog :custom-class="$style['el-dialog']" :title="isEdit ? '编辑' : '添加'" v-model="showEdit" width="600px">
    <a-form ref="editRef" :model="editItem" :label-col="{ span: 3 }" :rules="editRules">
      ...
      <a-form-item label="顺序" has-feedback name="Sorting">
        <a-input-number v-model:value="editItem.Sorting" :min="-2147483648" :max="2147483647"></a-input-number>
      </a-form-item>
    </a-form>
